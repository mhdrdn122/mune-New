...
import { getAllCategoriesAction, reorderCategoryAction, resetSuccess } from "../../redux/slice/categories/categoriesSlice";
...
export const CategoriesContainer = ({ categories, loading, setPage, page }) => {
  ...
  const getChangedPos = async (currentPos, newPos) => {
    if (newPos < 0 || newPos >= categories?.data?.length || currentPos === newPos) return;
