 ...
 */
const CouponsContainer = ({ show, handleClose, refresh, role }) => {
  ...
  const [deactivateCoupon, { isLoading: loadingDeactive }] = useDeactivateCouponMutation();
  ...
  return (
    <div>
      ...
      <DynamicForm
        ...
        onSubmit={async (values) =>
          await onUdateCouponSubmit(values, updateCoupon, () => setShowEidt(false), passedData?.id)
        }
      ...
      <AttentionModal
        ...
        onOk={async () =>
          await handleDelete(passedData?.id, () => setShowDelete(false), deleteCoupon)
        }
        show={showDelete}
      />
      ...
      <AttentionModal
        ...
        title={
          passedData?.is_active
            ? "إالغاء تنشيط الكوبون"
            : "تنشيط الكوبون"
        }
        onIgnore={() => setShowDeactive(false)}
        onOk={async () =>
          await handleDeactive(passedData?.id, deactivateCoupon, () => setShowDeactive(false))
        }
        show={showDeactive}
      />
