...
import { useAddNewsMutation } from "../../../redux/slice/news/newsApi";
...
const AddNews = ({
  ...
  const [addNews, { isLoading: loading, isError, error }] =
  useAddNewsMutation();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, {setErrors}) => {
      ...
      try {
        const result = await addNews(values).unwrap();
      ...
      <Modal show={show} onHide={handleClose} centered>
        <form onSubmit={formik.handleSubmit} autoComplete="off">
          <Modal.Header className="d-flex justify-content-center">
            <Modal.Title>إضافة خبر</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            ...
            {formik.errors.image && formik.touched.image ? (
              <div className="text-danger text-center my-2">{formik.errors.image}</div>
            ) : null}
            <TextField
              margin="dense"
              id="name_ar"
