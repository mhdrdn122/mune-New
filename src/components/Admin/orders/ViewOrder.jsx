            ...
            <div className='flex flex-column w-full gap-5'>
                ...
                <div className='flex gap-4 w-full' dir="rtl">
                    ...
                    <div className='flex gap-1 flex-wrap w-full'>
                        {
                            JSON.parse(order?.components)?.length > 0 ? JSON.parse(order?.components)?.map((comp)=><p>{comp?.name}</p>):
                            <p>لايوجد</p>
                        }
                    </div>
                </div>
                <div className='flex gap-4 w-full' dir="rtl">
                    ...
                    <div className='flex gap-1 flex-wrap w-full'>
                        {
                            JSON.parse(order?.toppings)?.length > 0 ? JSON.parse(order?.toppings)?.map((topping)=><p>{topping?.name},</p>):
                            <p>لايوجد</p>
                        }
                    </div>
                </div>
