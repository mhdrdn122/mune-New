  ...
  return (
    <div>
      ...
      <FormControl
        ...
        size="small"
        margin="dense"
        sx={{ width: "310px", mb: "20px", mr: "22px" }}
      >
        ...
        >
          <MenuItem value={"all"} dir="rtl"> الكل </MenuItem>
          <MenuItem value={"citySuperAdmin "} dir="rtl"> سوبر أدمن تابع لمدينة </MenuItem>
          <MenuItem value={"dataEntry "} dir="rtl"> مدخل بيانات </MenuItem>
        </Select>
      </FormControl>
            ...
            <tbody>
              {admins?.data?.length ? admins.data.map((admin, i) => (
                <tr key={i}>
                  ...
                  <td>
                    <IconButton onClick={() => handleShowAdmin(admin)}><FaEye /></IconButton>
                    {hasPermission(SuperPermissionsEnum.ADMIN_RESTAURANT_UPDATE) && (
                      <IconButton onClick={() => handleShowEdit(admin.id)}><EditOutlinedIcon /></IconButton>
                    )}
                    {hasPermission(SuperPermissionsEnum.ADMIN_RESTAURANT_DELETE) && (
                      <IconButton onClick={() => handleShowDelete(admin.id)}><DeleteIcon /></IconButton>
                    )}
                    {hasPermission(SuperPermissionsEnum.ADMIN_RESTAURANT_ACTIVE) && (
                      <IconButton onClick={() => handleShowDeactive(admin.id)}>
                        {admin.is_active === 1 ? <BlockOutlinedIcon /> : <CgUnblock />}
                      </IconButton>
                    )}
                  </td>
                </tr>
