...
const AppBarComponent = ({ Role }) => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      show: hasPermission(PermissionsEnum.TABLE_INDEX) && userData?.restaurant?.is_table === 1,
    },
    {
      title: "متابعة السائقين",
      to: "/admin/driver_tracker",
      icon: <FaTruckFast />,
      show: userData?.restaurant?.is_driver === 1,
    },
    {
      title: "الخدمات",
      to: "/admin/service",
      icon: <MdOutlineMiscellaneousServices />,
      show: hasPermission(PermissionsEnum.SERVICE_INDEX) && userData?.restaurant?.is_order === 1,
    },
    {
      title: "كوبونات",
      to: "/admin/coupon",
  ...
  return (
    ...
    >
      <Toolbar
        ...
        className="flex justify-between md:justify-start bg-[#2F4B26] rounded-b-3xl py-2 px-4"
        dir="rtl"
      >
        ...
        >
          {menuItems.filter((item) => item.show).map((item, index) => (
            <Link
              ...
              onMouseOver={(e) => {
                e.currentTarget.style.backgroundColor = "#3D5C2E";
